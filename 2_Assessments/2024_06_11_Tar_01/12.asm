...
; falta ver por que el mouse no responde en ocasiones
; falta ver porque se encicla con numeros grandes
.model SMALL                               
.STACK 512                                 
...
.DATA
    ...
    ; resultado
    iniArrNumAscii equ 0  ; primer posible indice del array rNumAscii
    finArrNumAscii equ 6  ; ultimo posible indice del array rNumAscii
    ...
    arrNumAscii db 00,00,00,00,00,00,00  ; array con valores a imprimir de numero E o F en ASCII
...
SelChResPr PROC ; selecciona caracter a imprimir del resultado, osea imprimir auxW01
    ; obtener ultimo posible indice del array arrNumAscii
    mov si, finArrNumAscii
    LoopSaveChPr: ; pequenno bucle para guardar los valores de numE a imprimir en numEAscii
        mov ax, auxW01  ; dividendo
        mov cl, 10
        div cl  ; al = cociente de (auxW01/10),  ah = residuo de (auxW01/10)
        mov arrNumAscii[si], ah  ; guardar residuo para imprimirlo
        mov ah, 00  ; limpia ah
        mov auxW01, ax  ; gracias a la limpia anterior, auxW01 = al
        dec si  ; disminiye en el indice de resultadoEnAscii
        cmp auxW01, 0  ; if mayor que 0
        jg LoopSaveChPr
        
    ; obtener primer posible indice del array arrNumAscii
    mov si, iniArrNumAscii
    LoopPrintChPr:
        mov ah, arrNumAscii[si]  ; obtiene el valor en arrNumAscii tal que indice = si
        cmp ah, 00  ; if ah == 0
        je AskLoopChPr
        mov auxB01, ah  ; ponemos en auxB01 el valor a imprimir
        add auxB01, 48  ; transforma el valor a caracter ASCII
        call PrintChar  ; llamamos al metodo que imprime
        inc contaVenta  ; incrementos la columna en la que se imprime en ventanita
        AskLoopChPr:
            inc si  ; incrementamos si
            cmp si, finArrNumAscii  ; if si < finArrNumAscii
            jl LoopPrintChPr
            
    ; fuera del loop hacemos este ya que estamos obligados a siempe imprimir el valor en arrNumAscii[finArrNumAscii]
    mov ah, arrNumAscii[si]  ; guardamos el valor en indice finArrNumAscii del array en ah
    mov auxB01, ah  ; guardamos el valor a imprimir
    add auxB01, 48  ; transforma el valor a caracter ASCII
    call PrintChar  ; llamamos al metodo que imprime
    inc contaVenta  ; incrementos la columna en la que se imprime en ventanita
