...
; solucionado lo de la impresion mas bonita
; falta anadirle soporte para que sea capaz de imprimir resultados negativos
; falta hacer que ciertas operaciones impriman solo 0, en vez de de 0/0 y hay ciertas divisiones que no deberia soportar
; falta documentar
.model SMALL                               
.STACK 512
.386                                 
...
SelChResPr PROC ; selecciona caracter a imprimir del resultado, osea imprimir auxW01
    ...
    LoopSaveChPr: ; peque?o bucle para guardar los valores de auxW01 a imprimir en arrNumAscii
        ...
        mov auxW01, ax  ; gracias a la divisi?n anterior, auxW01 = auxW01 / 10, guardamos ese cociente aqu?
        inc si  ; incrementa el ?ndice de arrNumAscii
        cmp si, sizeArrNumAscii  ; if si < sizeArrNumAscii
        jl LoopSaveChPr
        
    mov si, sizeArrNumAscii  ; si = 5
    dec si  ; decrementa en 1 para tener un ?ndice v?lido
    mov di, 0  ; usamos otro ?ndice, ya que es m?s f?cil contar de menos a m?s

    ; salta los ceros iniciales
    SkipLeadingZeros:
        cmp arrNumAscii[si], '0'
        jnz PrintLoop  ; si el caracter no es <<0>>, saltar al bucle de impresi?n
        dec si
        cmp si, 1  ; if si >= 1
